...
export async function GET(
  ...
  try {
    ...
    for (const rr of roomResults) {
      ...
      roomsHtml += `
        <h3 style="margin:16px 0 8px;font-size:14px;color:#1e3a5f;">${escXml(rr.roomName)} — ${rr.area.toFixed(1)} м²</h3>
        ...
        <p style="text-align:right;font-size:13px;font-weight:600;margin-top:4px;">Итого ${escXml(rr.roomName)}: ${fmtPrice(rr.subtotalAfterHeight)}</p>
      `;
    }
...
<body>
  ...
  <div style="text-align:right;">
    ...
    ${calc.totalArea > 0 ? `<p style="font-size:12px;color:#6b7280;margin-top:4px;">${fmtPrice(Math.round(total / calc.totalArea))}/м² | ${calc.totalArea.toFixed(1)} м²</p>` : ""}
  </div>
    ...
    * Расчёт предварительный. Точная стоимость определяется после замера.<br>
    ...
    return new NextResponse(html, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="KP-${estimate.clientName || "estimate"}-${estimate.publicId.slice(0, 8)}.html"`,
      },
    });
  } catch (error) {
